...
export function applyVoxelCurvedWorldTwinkle(material, {
  ...
  // Normalize against Lambert so the plateaus actually flatten light, rather
  // than merely tinting an otherwise continuous Lambert ramp. Bound the
  // terminator gain and retain shadow-map visibility from directDiffuse.
  float voxelToonRatio = clamp(voxelToonGrouped / max(voxelToonNdotL, 0.12), 0.65, 1.8);
