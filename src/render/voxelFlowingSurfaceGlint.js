  ...
  material.onBeforeCompile = function onBeforeCompile(shader, renderer) {
    ...
    shader.fragmentShader = shader.fragmentShader
      ...
      .replace(
        ...
        `#include <lights_fragment_end>\n#if NUM_DIR_LIGHTS > 0 && __VERSION__ >= 300\n  // Felt-like surfaces are grass / soil / foliage in the current voxel palette.\n  // Keep them matte; transparent water never reaches this diffuse voxel path.\n  float voxelGlintHardSurface = 1.0 - step(4.5, vVoxelGlintSurfaceKind) * step(vVoxelGlintSurfaceKind, 5.5);\n\n  vec2 voxelEdgeDistances = min(vVoxelGlintQuadUv, vec2(1.0) - vVoxelGlintQuadUv);\n  bool voxelVerticalEdge = voxelEdgeDistances.x < voxelEdgeDistances.y;\n  float voxelEdgeDistance = min(voxelEdgeDistances.x, voxelEdgeDistances.y);\n  float voxelAlongEdge = voxelVerticalEdge ? vVoxelGlintQuadUv.y : vVoxelGlintQuadUv.x;\n\n  // Keep the carrier narrow, with only a faint shoulder so it reads as a glint rather than a white outline.\n  float voxelEdgeWidth = max(fwidth(voxelEdgeDistance), 0.0015);\n  float voxelEdgeCore = 1.0 - smoothstep(voxelEdgeWidth * 0.25, voxelEdgeWidth * 1.05, voxelEdgeDistance);\n  float voxelEdgeShoulder = 1.0 - smoothstep(voxelEdgeWidth * 0.4, voxelEdgeWidth * 2.4, voxelEdgeDistance);\n\n  vec3 voxelLightDir = directionalLights[0].direction;\n  float voxelSunFacing = saturate(dot(geometryNormal, voxelLightDir));\n  vec3 voxelHalfDir = normalize(voxelLightDir + geometryViewDir);\n  float voxelReflection = pow(saturate(dot(geometryNormal, voxelHalfDir)), 30.0);\n  float voxelSunGate = smoothstep(0.24, 0.72, voxelSunFacing);\n\n  // Broad world-space cells sharply reduce the number of active candidates.\n  vec3 voxelCell = floor(vVoxelGlintWorldPosition / 1.55);\n  float voxelSeed = fract(sin(dot(voxelCell, vec3(12.9898, 78.233, 37.719))) * 43758.5453);\n  float voxelCandidate = smoothstep(0.982, 0.997, voxelSeed);\n\n  // The pulse center changes continuously with the view vector so the bright spot travels along the edge.\n  float voxelViewPhase = dot(geometryViewDir, vec3(0.31, 0.53, 0.79));\n  float voxelPulseCenter = fract(voxelSeed * 1.73 + voxelViewPhase * 0.86 + voxelSunFacing * 0.17);\n  float voxelPulseDistance = abs(voxelAlongEdge - voxelPulseCenter);\n  voxelPulseDistance = min(voxelPulseDistance, 1.0 - voxelPulseDistance);\n  float voxelFootprint = saturate(length(fwidth(vVoxelGlintWorldPosition)) * 2.0);\n  float voxelPulseWidth = mix(0.075, 0.14, voxelFootprint);\n  float voxelPulse = 1.0 - smoothstep(voxelPulseWidth * 0.45, voxelPulseWidth, voxelPulseDistance);\n  voxelPulse *= voxelPulse;\n\n  // Corners can catch a compact flash, but they share the same sparse candidate gate.\n  float voxelCornerDistance = max(voxelEdgeDistances.x, voxelEdgeDistances.y);\n  float voxelCornerWidth = max(fwidth(voxelCornerDistance) * 1.45, 0.002);\n  float voxelCorner = 1.0 - smoothstep(voxelCornerWidth * 0.3, voxelCornerWidth * 1.2, voxelCornerDistance);\n\n  float voxelGlintCore = max(voxelEdgeCore * voxelPulse, voxelCorner * 0.62);\n  float voxelGlint = voxelGlintHardSurface\n    * voxelCandidate\n    * voxelSunGate\n    * voxelReflection\n    * voxelGlintCore;\n\n  // Compact bright core plus a very weak warm shoulder. The footprint term preserves some\n  // far-screen energy without an explicit camera-distance boost or another draw call/pass.\n  float voxelFarEnergy = mix(1.0, 1.45, voxelFootprint);\n  reflectedLight.directSpecular += directionalLights[0].color\n    * vec3(1.0, 0.965, 0.88)\n    * voxelGlint\n    * 1.08\n    * voxelFarEnergy;\n  reflectedLight.directSpecular += directionalLights[0].color\n    * vec3(1.0, 0.88, 0.72)\n    * voxelGlintHardSurface\n    * voxelCandidate\n    * voxelSunGate\n    * voxelReflection\n    * voxelPulse\n    * voxelEdgeShoulder\n    * 0.055\n    * voxelFarEnergy;\n#endif`
      );
  };
