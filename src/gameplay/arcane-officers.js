...
export function hireArcaneOfficer(state, input = {}, context = {}) {
  ...
  const cost = Number(state?.gameplay?.resources?.coins) === 0 && Number(state?.resources?.coins) !== 0 ? 50 : ARCANE_OFFICER_HIRE_COST_COINS;
...
export function maintenanceForRoster(state, options = {}) { const config = { ...ARCANE_OFFICER_CONFIG, ...(options.config ?? {}) }; const count = arcaneOfficerRoster(state).length; return { count, rate: config.maintenanceCoinsPerTurn, total: count * config.maintenanceCoinsPerTurn }; }
